 ...
 */
 ...
 (function($){
 	$.ajax({
		...
		success: function(r){
			if(r !== '-1' )
				...
				setTimeout(function(){
					$('#ch_main_wrapper').fadeIn('slow',function(){});
	 ...
	 $.fn.prepareChatContext = function(){
		...
		if(sessionStorage.getItem(chat_app_obj.ss_hidden)){
			chat_app_obj.hiddenChatBoxObjArr = JSON.parse(sessionStorage.getItem(chat_app_obj.ss_hidden));
			$.each(chat_app_obj.hiddenChatBoxObjArr, function(i,j){
				if(chat_app_obj.chatBoxArr[j[0]] && chat_app_obj.chatBoxArr[j[0]] instanceof chatBox){
					chat_app_obj.hiddenChatBoxObjArr.splice(i,1);
					chat_app_obj.hiddenChatBoxObjArr.push([chat_app_obj.chatBoxArr[j[0]].recevier, chat_app_obj.chatBoxArr[j[0]]]);
				}
			})
		}
		chat_app_obj.manageHiddenBox();
		/**
		...
		$.each(chat_app_obj.chatBoxArr, function(){
			if(this instanceof chatBox){				
				// ~~  currently closedas client don't want the state saving feature
				this.restore(); 
				//~-
				this.prepare();
			}
 ...
 chatApp = function( user_id, project_context){
	 ...
	 */			
	  ...
	  * Hidden message dom
	  */
	 this.hidden_msg_dom = $('div.ch_add_back');	 
	 /* Toogle the names*/
	 this.hidden_msg_dom.find('.add_icon').on('click',function(){
	 	$(this).closest('div.ch_add_back').find('.ch_up_panel').slideToggle();
	 })
	 this.hidden_msg_dom.find('.ch_add_back').on('blur',function(){

	 	$(this).closest('div.ch_add_back').find('.ch_up_panel').slideUp();
	 })
	 
	 /** 
	  * Open chat box from hidden state
	  */
	 $(document).on('click','.ch_hidden_user_id', function(){
	 	var userid = $(this).data('hiddenuserid');
		$.each(chat_app_obj.hiddenChatBoxObjArr, function(i,j){
			if(j[1] instanceof chatBox && j[1].receiver == userid){
				j[1].parent_rel_dom.click();
			}
		})
	 });
	  $(document).on('click','.ch_hidden_cross', function(e){
		e.stopImmediatePropagation(); 
	 	var userid = $(this).data('hiddenuserid');
		$.each(chat_app_obj.hiddenChatBoxObjArr, function(i,j){
			if(j[1] instanceof chatBox && j[1].receiver == userid){
				chat_app_obj.hiddenChatBoxObjArr.splice(i,1);
				chat_app_obj.activeChatBoxObjArr.push([j[1].receiver, j[1]]);
				j[1].parent_rel_dom.click();
				j[1].dom.find('.cross_fb').click();
			}
		})
	 	return false;	
	 })
 ...
 chatApp.prototype.manageHiddenBox = function(){	 
 	if(this.hiddenChatBoxObjArr.length == 0){
		this.hidden_msg_dom.css('display','none');
	}else{
		this.hidden_msg_dom.find('.ch_hidden_count').text(this.hiddenChatBoxObjArr.length).end().css('display','inline-block');
		chat_app_obj.hidden_msg_dom.find('.ch_up_panel').html('');		
		$.each(this.hiddenChatBoxObjArr, function(i,j){
			if(j[1] instanceof chatBox){
				chat_app_obj.hidden_msg_dom.find('.ch_up_panel').append('<p class="fb_profiler ch_hidden_user_id ch_hiddden_user-'+j[1].receiver+'" data-hiddenuserid="'+j[1].receiver+'">'+j[1].dom.data('username')+'<span class="cross_fb ch_hidden_cross" data-hiddenuserid="'+j[1].receiver+'"></span></p>');
			}
		})
	}
 }
...
chatApp.prototype.save = function(){
 ...
 chatBox = function(sender, receiver){
	...
	//this.prepare();
	this.onfocus = false;// convey as the user is typing in this window
	 ...
	 if(e.keyCode === 13 && e.shiftKey !== true){
		$.ajax({
			...
			beforeSend: function(){
				...
				obj.dom.find('.ch_content_whole .typing').remove();
				obj.dom.find('.ch_content_whole').append(html);
				obj.moveScrollerDown();
			},
			success:function(r){
				$('.message_content').css({opacity:1}).attr('title', r);
   ...
   */
   var source = new EventSource(app_url+'&f=eventsource');
	...
	if(res.hasOwnProperty("response")){
		$.each(res.response, function(v,m){
			$.each(m, function(uid, message){
				var chat_boxobj = null;
				$.each(chat_app_obj.chatBoxArr, function(i,j){
					if(j instanceof chatBox && j.receiver == uid){
						var obj = j;
						obj.new_msg_count += parseInt(message.length);
						if(obj.status == 2 && obj.new_msg_count > 0){// the message bix was 
							obj.dom.find('.ch_new_msg_count').text(obj.new_msg_count).css({'padding': '2px', 'display': 'block'});
							obj.parent_rel_dom.find('.ch_inread_cnt').text('('+obj.new_msg_count+')')
						}
						if( (obj.status == 3 || obj.status == 0) && obj.new_msg_count > 0){// the message bix was 
							//chat_app_obj.dom.find('.ch_total_inrd_cnt').text('('+obj.new_msg_count + ')').css({'padding': '2px'});
							var old_ttl_cnt = chat_app_obj.dom.find('.ch_total_inrd_cnt').text().replace('(','').replace(')');
							if(!old_ttl_cnt){
								old_ttl_cnt =0
							}
							old_ttl_cnt  = parseInt(old_ttl_cnt)+obj.new_msg_count;
							if(chat_app_obj.openStatus){								
								chat_app_obj.dom.find('.ch_total_inrd_cnt').text('('+old_ttl_cnt + ')').css({'padding': '2px'});
							}
						...
						$.each(message, function(m_index, m_dody){
							if(obj.dom.find('.ch_content_whole .typing').length > 0){								
								$(m_dody).insertBefore(obj.dom.find('.ch_content_whole .typing'));
							}else{
								obj.dom.find('.ch_content_whole').append(m_dody);
							}
							obj.moveScrollerDown();
						});
						/**
